        ...
        if num_filas > 0:
            ...
            if tipo == 'Sell' or tipo == 'Salidas':
                ...
                try:
                    ...
                    pass
                #Verificar monto disponible en banco
                try:
                    disponibilidad = WebDriverWait(driver,10).until(
                        EC.presence_of_element_located((By.XPATH,'//*[@id="contenedor1"]/div[3]/div[2]/div'))
                    ).text
                    saldo = str((disponibilidad.split(','))[0]).replace('.','')
                    print(saldo)

                except:
                    driver.quit

                if int(saldo) <= int(monto1):
                    print('Fondos insuficientes para realizar transferencia')
                    driver.switch_to.window(driver.window_handles[0])
                    offset +=2
                    continue

                #Si saldo es suficiente, ir a transferencias    
                try:
                ...
                try:
                    ...
                    driver.quit
                    bot()
                #Chequear estatus de transacción
                try:
                    estatus = WebDriverWait(driver,20).until(
                ...
                if 'Rechazada' in estatus:
                    print('Operación rechazada por banco, intentando de nuevo')
                    driver.switch_to.window(driver.window_handles[0])
                    continue
